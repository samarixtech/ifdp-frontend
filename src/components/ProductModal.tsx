...
const mockItemGallery = [
  "https://plus.unsplash.com/premium_photo-1675252369719-dd52bc69c3df?w=600&auto=format&fit=crop&q=60",
...
const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const handleAddOnToggle = (
    addOn: AddOn & { groupId: string },
    group: (typeof mockAddOnGroups)[0]
  ) => {
  ...
  const nextImage = () =>
    setCurrentImageIndex((i) => (i + 1) % mockItemGallery.length);
  const prevImage = () =>
    setCurrentImageIndex((i) => (i - 1 + mockItemGallery.length) % mockItemGallery.length);
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
              ...
              {mockVariations.map((variation) => (
                ...
                >
                  ...
                  <div className="flex items-center">
                    <span className="font-semibold mr-2">{currency} {variation.price.toLocaleString()}</span>
                    <input
                      type="radio"
                      name="variation"
              ...
              <div className="space-y-2">
                {group.items.map((addOn) => {
                  ...
                  const isMaxReached =
                    ...
                    selectedAddOns.filter((a) => a.groupId === group.id).length >= group.maxSelection &&
                    !isSelected;
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        <span className="font-semibold mr-2">
                          {addOn.price > 0 ? `+ ${currency} ${addOn.price.toLocaleString()}` : "FREE"}
                        </span>
                        <input
          ...
          <div className="mb-2">
            ...
            <textarea
              ...
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition min-h-[60px] text-sm resize-none"
            ></textarea>
          </div>
        </div>
        ...
        <div className="bg-white border-t border-gray-100 p-3 flex justify-between items-center shadow-2xl flex-shrink-0">
          ...
          <div className="flex items-center space-x-2 border border-gray-300 rounded-full p-0.5">
            ...
            <button
              ...
              className="p-1 bg-green-100 text-green-600 hover:bg-green-200 rounded-full transition"
            >
              <Plus size={18} />
            </button>
          </div>
          ...
          >
            Add {quantity} to Cart ({currency} {finalTotal.toLocaleString()})
          </button>
        </div>
      </div>
    </div>
