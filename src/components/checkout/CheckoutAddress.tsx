...
const CheckoutAddress: React.FC = () => {
    ...
    const addresses = [
        ...
        { id: 2, label: 'Work', address: 'Office 45, Blue Area, F8', icon: MapPin, selected: false },
    ];
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            <div className="flex justify-between items-center mb-4">
                ...
                <button className="text-sm text-blue-600 font-semibold flex items-center hover:text-blue-700 transition">
                ...
                {addresses.map(addr => (
                    <div 
                        ...
                        className={`flex items-start p-3 rounded-lg border-2 transition cursor-pointer 
                            ${addr.selected ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                    >
                        <addr.icon className={`w-5 h-5 mt-1 mr-3 ${addr.selected ? 'text-blue-600' : 'text-gray-500'}`} />
                        <div>
